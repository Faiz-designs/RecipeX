...
export default function Contact() {
  ...
  return (
    <>
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-red-600 via-red-500 to-rose-600 py-20 md:py-28">
        ...
        <div className="relative max-w-3xl mx-auto text-center px-4">
          ...
          <p className="text-lg text-red-100/90 leading-relaxed max-w-xl mx-auto">
            {t('contact.title')}
          </p>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-10 mb-16">
          <div className="glass-card rounded-2xl shadow-xl p-8">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-11 h-11 bg-gradient-to-br from-red-500 to-red-700 rounded-xl flex items-center justify-center text-lg shadow-md">✉️</div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" name="name" value={form.name} onChange={handleChange} required className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-stone-700/80 border border-stone-200/80 dark:border-stone-600/60 text-stone-800 dark:text-stone-100 placeholder-stone-400 dark:placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-red-400/60 focus:border-red-400 transition-all shadow-sm" placeholder={t('contact.name')} />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea name="message" value={form.message} onChange={handleChange} required rows="5" className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-stone-700/80 border border-stone-200/80 dark:border-stone-600/60 text-stone-800 dark:text-stone-100 placeholder-stone-400 dark:placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-red-400/60 focus:border-red-400 transition-all resize-vertical shadow-sm" placeholder={t('contact.message')} />
              </div>
              <button type="submit" className="w-full py-3 btn-glass btn-glass-red rounded-xl active:scale-[0.98]">{t('contact.submit')}</button>
            </form>
          </div>

          <div>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-11 h-11 bg-gradient-to-br from-red-500 to-red-700 rounded-xl flex items-center justify-center text-lg shadow-md">❓</div>
              ...
              {faqItems.map((item, i) => (
                <div key={i} className="glass-card rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
                  <button onClick={() => setExpanded(expanded === i ? null : i)} className="w-full flex items-center justify-between p-5 text-left hover:bg-stone-50/80 dark:hover:bg-stone-700/40 transition-colors">
                    ...
                    <span className={`shrink-0 w-7 h-7 rounded-full bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/50 dark:to-rose-900/50 flex items-center justify-center text-sm font-bold text-red-600 dark:text-red-400 transition-all duration-300 border border-red-200/60 dark:border-red-700/50 ${expanded === i ? 'rotate-180 bg-gradient-to-br from-red-100 to-rose-100 dark:from-red-800/60 dark:to-rose-800/60' : ''}`}>
