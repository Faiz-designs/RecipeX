    ...
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-100/80 text-sm">{t('history.scansSaved', { count: history.length })}</p>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div>
            <button onClick={() => setSelected(null)} className="flex items-center gap-1.5 text-sm text-red-600 dark:text-red-400 font-medium mb-5 hover:text-red-700 dark:hover:text-red-300 transition-colors group">
            ...
            <div className="glass-card rounded-2xl shadow-lg p-5 md:p-7 mb-6">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 bg-gradient-to-br from-red-500 to-red-700 rounded-xl flex items-center justify-center shadow-md text-lg">📊</div>
                <div>
            ...
            {r.scan_summary?.items && (
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-8 h-8 bg-gradient-to-br from-red-500 to-red-700 rounded-lg flex items-center justify-center text-xs shadow-sm">🥬</div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {r.scan_summary.items.map(item => (
                    <div key={item.id} className="glass-card rounded-xl shadow-sm p-3.5 hover:shadow-md transition-shadow">
                      ...
                      <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full inline-block mt-1.5 ${item.freshness_status === 'Fresh' ? 'bg-red-50 dark:bg-red-900/60 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-700/50' : item.freshness_status === 'Slightly Aged' ? 'bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700/50' : 'bg-red-100 dark:bg-red-900/60 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-700/50'}`}>{item.freshness_status}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="min-w-0 flex items-center gap-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/40 dark:to-rose-900/40 rounded-xl flex items-center justify-center text-lg shrink-0">🥗</div>
                      <div>
