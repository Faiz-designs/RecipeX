    ...
    <div className="mb-8">
      ...
      <div className="space-y-4">
        {substitutions.map((s, i) => (
          <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
            <div className="p-5">
              <div className="flex flex-col sm:flex-row items-start justify-between gap-3 mb-3">
                <div className="flex items-center flex-wrap gap-x-3 gap-y-1">
                  <span className="text-lg font-bold text-slate-800 dark:text-slate-100">{s.original_vegetable_name}</span>
              ...
              {s.recipe_update && <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 italic">📝 {s.recipe_update.updated_ingredient_line}</p>}
            </div>
          </div>
        ))}
      </div>
