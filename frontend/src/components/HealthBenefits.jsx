      ...
      <div className="space-y-4">
        {health_benefits.map((veg) => (
          <div key={veg.vegetable_id} className="glass-card rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div className="bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-900/30 dark:to-rose-900/30 px-5 py-3 border-b border-stone-100 dark:border-stone-700">
              <h3 className="font-bold text-stone-800 dark:text-stone-100 text-lg">{veg.vegetable_name}</h3>
            </div>
            <div className="p-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {veg.benefits?.map((b, i) => (
                <div key={i} className="bg-red-50/50 dark:bg-red-900/15 border border-red-100 dark:border-red-800/40 rounded-xl p-3.5 hover:shadow-sm transition-shadow">
                  <div className="flex items-center gap-1.5 mb-1">
                    <span className="w-2 h-2 rounded-full bg-red-500 shrink-0" />
                    <h4 className="font-bold text-red-700 dark:text-red-300 text-sm">{b.benefit}</h4>
                  </div>
