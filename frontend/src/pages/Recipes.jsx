...
export default function Recipes() {
  ...
  const difficultyConfig = {
    easy: { label: t('recipes.easy'), badge: 'bg-red-50 dark:bg-red-900/60 text-red-700 dark:text-red-300', header: 'from-red-600 to-red-700' },
  ...
  if (loading) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="relative overflow-hidden bg-gradient-to-br from-red-600 via-red-500 to-red-400 rounded-2xl mb-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3].map(i => (
            <div key={i} className="glass-card rounded-2xl overflow-hidden shadow-sm">
              <div className="h-20 bg-gradient-to-r from-stone-200 via-stone-300 to-stone-200 dark:from-stone-700 dark:via-stone-600 dark:to-stone-700 animate-shimmer" />
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="glass-card rounded-2xl p-8 text-center shadow-sm">
          ...
          <button onClick={() => window.location.reload()} className="px-6 py-2.5 btn-glass btn-glass-red rounded-xl active:scale-[0.98]">{t('recipes.tryAgain')}</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-red-600 via-red-500 to-red-400 animate-gradient py-20 md:py-28">
        <div className="absolute inset-0 bg-white/5" />
        ...
        <div className="relative max-w-3xl mx-auto text-center px-4">
          ...
          <p className="text-lg text-red-100/90 leading-relaxed max-w-xl mx-auto font-medium">
            {t('recipes.subtitle')}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex gap-2 mb-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-white dark:bg-stone-800/90 border border-stone-200/80 dark:border-stone-700/60 rounded-xl text-sm text-stone-700 dark:text-stone-200 focus:outline-none focus:ring-2 focus:ring-red-400/50 focus:border-red-400 transition-all duration-200 shadow-sm placeholder:text-stone-400 dark:placeholder:text-stone-500"
            />
          </div>
          <button
            onClick={() => setShowPantry(!showPantry)}
            className={`px-4 py-2.5 rounded-xl text-sm font-bold border transition-all duration-300 active:scale-[0.98] ${showPantry ? 'bg-red-600 text-white border-red-600 shadow-md shadow-red-600/20 ring-1 ring-red-600/20' : 'bg-white dark:bg-stone-800/90 text-stone-600 dark:text-stone-300 border-stone-200/80 dark:border-stone-700/60 hover:border-red-300 dark:hover:border-red-600 hover:shadow-sm'}`}
          >
            🧺 {t('recipes.myPantry')}
          </button>
        </div>
          ...
          <div className="glass-card rounded-2xl p-5 mb-6 shadow-sm hover:shadow-lg transition-all duration-300">
            ...
            {pantry.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {pantry.map((item, i) => (
                  <span key={i} className="inline-flex items-center gap-1.5 text-xs bg-red-50 dark:bg-red-900/40 text-red-700 dark:text-red-300 border border-red-200/80 dark:border-red-700/60 px-3 py-1.5 rounded-full font-semibold shadow-sm">
                    {item}
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 px-3 py-2 bg-stone-50 dark:bg-stone-700/50 border border-stone-200/80 dark:border-stone-600/60 rounded-lg text-sm text-stone-700 dark:text-stone-200 focus:outline-none focus:ring-2 focus:ring-red-400/50 focus:border-red-400 transition-all duration-200"
              />
              <button onClick={handleAddPantry} className="px-4 py-2 btn-glass btn-glass-red rounded-lg text-sm active:scale-[0.98]">{t('shoppingList.addItem')}</button>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {pantryMatchOptions.map(option => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-xs font-bold border transition-all duration-200 ${pantryFilter === option ? 'bg-red-600 text-white border-red-600 shadow-sm ring-1 ring-red-600/20' : 'bg-white dark:bg-stone-700 text-stone-500 dark:text-stone-400 border-stone-200 dark:border-stone-600 hover:border-red-300 dark:hover:border-red-600 hover:shadow-sm'}`}
                    >
                      {option}
                    </button>
                  ))}
        ...
        <div className="flex flex-wrap gap-3 mb-6 p-3 glass-card rounded-2xl">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {[{ key: 'any', label: t('filters.anyTime') }, { key: 'under15', label: t('filters.under15') }, { key: 'under30', label: t('filters.under30') }, { key: 'over30', label: t('filters.over30') }].map(opt => (
              <button key={opt.key} onClick={() => setPrepTime(opt.key)} className={`px-3 py-1 rounded-lg text-xs font-bold border transition-all duration-200 active:scale-[0.97] ${prepTime === opt.key ? 'bg-red-600 text-white border-red-600 shadow-sm ring-1 ring-red-600/20' : 'bg-white dark:bg-stone-700 text-stone-500 dark:text-stone-400 border-stone-200 dark:border-stone-600 hover:border-red-300 dark:hover:border-red-600 hover:shadow-sm'}`}>
                {opt.label}
              </button>
            ))}
          </div>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {[{ key: 'all', label: 'All' }, { key: 'vegetarian', label: t('filters.vegetarian') }, { key: 'vegan', label: t('filters.vegan') }, { key: 'glutenFree', label: t('filters.glutenFree') }, { key: 'lowCarb', label: t('filters.lowCarb') }].map(opt => (
              <button key={opt.key} onClick={() => setDiet(opt.key)} className={`px-3 py-1 rounded-lg text-xs font-bold border transition-all duration-200 active:scale-[0.97] ${diet === opt.key ? 'bg-red-600 text-white border-red-600 shadow-sm ring-1 ring-red-600/20' : 'bg-white dark:bg-stone-700 text-stone-500 dark:text-stone-400 border-stone-200 dark:border-stone-600 hover:border-red-300 dark:hover:border-red-600 hover:shadow-sm'}`}>
                {opt.label}
              </button>
            ))}
          </div>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {[{ key: 'all', label: t('filters.allSeasons') }, { key: 'spring', label: t('filters.spring') }, { key: 'summer', label: t('filters.summer') }, { key: 'fall', label: t('filters.fall') }, { key: 'winter', label: t('filters.winter') }].map(opt => (
              <button key={opt.key} onClick={() => setSeason(opt.key)} className={`px-3 py-1 rounded-lg text-xs font-bold border transition-all duration-200 active:scale-[0.97] ${season === opt.key ? 'bg-red-600 text-white border-red-600 shadow-sm ring-1 ring-red-600/20' : 'bg-white dark:bg-stone-700 text-stone-500 dark:text-stone-400 border-stone-200 dark:border-stone-600 hover:border-red-300 dark:hover:border-red-600 hover:shadow-sm'}`}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16 glass-card rounded-2xl shadow-sm">
            ...
            <button onClick={handleClearFilters} className="inline-flex items-center gap-2 btn-glass btn-glass-red px-6 py-3 rounded-xl active:scale-[0.98] text-sm">
              ✕ Clear All Filters
            </button>
          </div>
        ) : (
