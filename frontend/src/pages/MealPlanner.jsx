          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-100/80 text-sm">{t('mealPlanner.subtitle')}</p>
              </div>
            </div>
            <button
              ...
              {dayOrder.map((dayEn, di) => {
                ...
                return (
                  <div key={dayEn} className="glass-card rounded-2xl overflow-hidden shadow-sm">
                    <div className="bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-900/30 dark:to-rose-900/30 px-5 py-3 border-b border-stone-200/60 dark:border-stone-700/50 flex items-center justify-between">
                        ...
                        return (
                          <div key={mk} className="glass-card rounded-xl p-3">
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-wide">{t(`mealPlanner.${mk}`)}</span>
                ...
                <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {aiPlan.weekly_grocery_list.map((cat, i) => (
                    <div key={i} className="glass-card rounded-xl p-3">
                      <p className="text-xs font-bold text-red-600 dark:text-red-400 uppercase mb-1.5">{cat.category}</p>
          ...
          <>
                  ...
                  {MEALS.map(meal => (
                    <tr key={meal} className="border-t border-stone-200/40 dark:border-stone-700/40">
                      ...
                      {DAYS.map(day => {
                        ...
                        return (
                          <td key={day} className="p-2 text-center border-r border-stone-200/40 dark:border-stone-700/40 last:border-r-0 relative hover:bg-stone-50/50 dark:hover:bg-stone-700/20 transition-colors">
                            {recipe ? (
                            ...
                            {isSelecting && allRecipes.length > 0 && (
                              <div className="absolute z-20 top-full left-1/2 -translate-x-1/2 mt-1.5 glass-card rounded-xl shadow-xl p-2 min-w-[200px] max-h-56 overflow-y-auto">
                                ...
                                {allRecipes.map((r, i) => (
                                  <button key={i} onClick={() => setMeal(day, meal, r.name)} className="block w-full text-left text-xs p-2.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-stone-600 dark:text-stone-300 transition-colors font-medium">
                                    {r.name}
                                  </button>
                                ))}
                              </div>
            ...
            <div className="mt-8 glass-card rounded-2xl shadow-sm p-6 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-2.5 mb-4">
                <div className="w-8 h-8 bg-gradient-to-br from-red-500 to-red-600 rounded-lg flex items-center justify-center text-sm shadow-sm">🍽️</div>
              ...
              {allRecipes.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {allRecipes.map((r, i) => (
                    <span key={i} className="text-xs bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-900/30 dark:to-rose-900/30 text-red-700 dark:text-red-300 border border-red-200/60 dark:border-red-700/50 px-3 py-1.5 rounded-full font-medium shadow-sm">{r.name}</span>
                  ))}
                </div>
              )}
            </div>
