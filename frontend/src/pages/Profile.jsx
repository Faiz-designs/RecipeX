  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-br from-red-600 via-red-500 to-rose-600 rounded-2xl p-6 md:p-8 mb-8 text-white shadow-xl shadow-red-600/10">
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-red-500 rounded-full border-2 border-red-700 flex items-center justify-center shadow-sm">
              <span className="text-white text-xs">✓</span>
            </div>
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">{user.full_name || user.username}</h1>
            <p className="text-red-100/80 text-sm flex items-center gap-1.5">@{user.username}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3 mt-6">
        ...
        <Link to="/scan" className="px-6 py-2.5 btn-glass btn-glass-red rounded-xl active:scale-[0.98] flex items-center gap-2">
          {t('nav.scan')}
        </Link>
      </div>
    </div>
