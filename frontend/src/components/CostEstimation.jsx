    ...
    <div className="mb-8">
      ...
      {improvements?.estimated_total_cost && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30 border border-amber-200 dark:border-amber-700/40 rounded-2xl p-4 mb-4">
          ...
          {improvements.leftover_recipe_suggestion && (
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-3 pt-3 border-t border-amber-200/50 dark:border-amber-700/30 flex items-start gap-1.5">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {cost_estimation.map((veg) => (
          <div key={veg.vegetable_id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
            ...
            <div className="space-y-1.5">
              ...
              {veg.price_seasonality && (
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1.5 pt-1.5 border-t border-slate-100 dark:border-slate-700">{veg.price_seasonality}</p>
              )}
              {veg.budget_tip && (
