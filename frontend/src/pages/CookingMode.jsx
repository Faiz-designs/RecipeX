...
export default function CookingMode() {
  ...
  useEffect(() => {
    ...
    window.__NUTRIVIZEN_RECIPE__ = null
    if (r && typeof r === 'object') setRecipe(r)
  }, [])
  ...
  useEffect(() => {
    if (window.__NUTRIVIZEN_AUTOSPEAK__ && steps[0]) {
      ...
      setTimeout(() => speak(steps[0]), 500)
    }
  }, [steps])

  useEffect(() => {
    if (speaking && steps[currentStep]) speak(steps[currentStep])
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-stone-900 via-stone-800 to-stone-900">
        <div className="glass-card rounded-2xl p-8 shadow-xl text-center max-w-sm">
          ...
          <p className="text-white/80 text-xl font-medium mb-2">No recipe selected</p>
          <p className="text-stone-500 text-sm">Use the AI Assistant (🤖) to generate a recipe</p>
        </div>
      </div>
    )
  }
  ...
  if (steps.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-stone-900 via-stone-800 to-stone-900">
        <div className="glass-card rounded-2xl p-8 shadow-xl text-center max-w-sm">
          <p className="text-4xl mb-4">🍳</p>
          <p className="text-white/80 text-xl font-medium mb-2">{recipe.name || 'Recipe'}</p>
          <p className="text-stone-500 text-sm">This recipe has no steps</p>
        </div>
      </div>
    )
  }

  const toggleIngredient = (i) => setChecked(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i])
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-stone-900 via-stone-800 to-stone-900 text-white flex flex-col">
        <header className="glass-nav px-6 py-4 flex items-center justify-between sticky top-0 z-30">
          ...
          <button onClick={() => navigate(-1)} className="px-4 py-2 bg-white/10 backdrop-blur-sm rounded-xl text-sm font-semibold hover:bg-white/20 transition-all border border-white/10">{t('cookingMode.exit')}</button>
        </header>
            ...
            <div className="flex items-center gap-1 flex-1 overflow-x-auto scrollbar-none">
              {steps.map((_, i) => (
                <button key={i} onClick={() => setCurrentStep(i)}
                  className={`shrink-0 rounded-full transition-all duration-300 ${i === currentStep ? 'w-8 h-1.5 bg-lime-400 shadow-lg shadow-lime-400/30' : i < currentStep ? 'w-2 h-2 bg-emerald-500/60 hover:bg-emerald-400' : 'w-2 h-2 bg-stone-600/60 hover:bg-stone-500'}`}
          ...
          <aside className="lg:w-80 glass-card p-6 border-b lg:border-b-0 lg:border-r">
            ...
            <div className="space-y-2.5">
              {ingredients.map((ing, i) => (
                <button key={i} onClick={() => toggleIngredient(i)}
                  className={`w-full text-left px-4 py-3 rounded-xl text-base flex items-center gap-3 transition-all ${checked.includes(i) ? 'bg-slate-700/50 text-stone-400 line-through' : 'bg-slate-700/30 hover:bg-slate-700/60 border border-slate-700/40 hover:border-slate-600/60'}`}>
            ...
            <div className="mt-8 pt-6 border-t border-slate-700/60">
              ...
              <div className="flex gap-2 flex-wrap">
                {[1, 2, 3, 5, 10, 15].map(m => (
                  <button key={m} onClick={() => startTimer(m)} className="px-3 py-2 bg-slate-700/50 rounded-xl text-sm hover:bg-slate-600/60 transition-all border border-slate-600/40 hover:border-lime-500/40 hover:shadow-lg hover:shadow-lime-500/10 active:scale-95">{m} min</button>
                ))}
              </div>
              {timerSeconds > 0 && (
          ...
          <main className="flex-1 flex flex-col items-center justify-center p-6 md:p-10">
            <div className="max-w-2xl w-full">
              ...
              <div className="glass-card rounded-2xl p-8 md:p-10 mb-8 min-h-[220px] flex items-center justify-center shadow-xl relative overflow-hidden group">
                ...
                <p className="text-2xl md:text-3xl leading-relaxed text-center font-medium text-stone-100 relative z-10">{steps[currentStep]}</p>
              </div>
              ...
              <div className="flex justify-between gap-4">
                ...
                {currentStep === steps.length - 1 ? (
                  <button onClick={() => navigate(-1)} className="flex-1 px-6 py-4 btn-glass btn-glass-lime rounded-xl text-lg font-semibold active:scale-[0.98]">{t('cookingMode.done')} ✓</button>
                ) : (
                  <button onClick={() => setCurrentStep(s => Math.min(steps.length - 1, s + 1))}
                    className="flex-1 px-6 py-4 btn-glass btn-glass-lime rounded-xl text-lg font-semibold active:scale-[0.98]">{t('cookingMode.next')} →</button>
                )}
              </div>
              ...
              <div className="mt-6 glass-card rounded-2xl p-5 shadow-lg">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3">
                    <button onClick={() => speaking ? stopSpeaking() : speak(steps[currentStep])}
                ...
                {voiceInput && <div className="mt-3 text-sm text-stone-300 bg-slate-700/40 backdrop-blur-sm rounded-xl px-4 py-2.5 border border-slate-600/40">🗣 "{voiceInput}"</div>}
