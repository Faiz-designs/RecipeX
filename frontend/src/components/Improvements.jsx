  ...
  return (
    <div className="mb-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">AI Improvement Suggestions</h2>
      </div>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        ...
        <div className="p-5 space-y-5">
          {improvements.nutritional_gaps?.length > 0 && (
            <div>
              <p className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-red-400" /> Nutritional Gaps</p>
          ...
          {improvements.suggested_add_ons?.length > 0 && (
            <div>
              <p className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-blue-400" /> Suggested Add-ons</p>
              <div className="space-y-2.5">
                {improvements.suggested_add_ons.map((item, i) => (
                  <div key={i} className="bg-blue-50/50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/50 rounded-xl p-3.5">
                    ...
                    <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Adds: {item.nutrient_it_adds}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {improvements.cooking_technique_upgrades && Object.entries(improvements.cooking_technique_upgrades).some(([, v]) => v) && (
            <div>
              <p className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-purple-400" /> Cooking Upgrades</p>
          ...
          {improvements.next_scan_suggestion && (
            <div className="bg-emerald-50/50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 rounded-xl p-3.5">
              <p className="text-xs font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wide mb-1">Next Scan Suggestion</p>
          ...
          {improvements.overall_verdict && (
            <div className="bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600 rounded-xl p-3.5">
              <p className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide mb-1">Verdict</p>
