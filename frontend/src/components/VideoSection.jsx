      ...
      <div className="glass-card rounded-2xl p-5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t('videoSection.learnWithVideo')}</p>
          </div>
          <a
        ...
        {difficulty && (
          <div className="flex gap-2 mt-3 pt-3 border-t border-red-200/40 dark:border-red-800/40">
            <span className="text-xs text-slate-500 dark:text-slate-400">{t('videoSection.suggested')}:</span>
            {['Easy', 'Authentic', 'Quick'].map(tag => (
              <a key={tag} href={`https://www.youtube.com/results?search_query=${searchQuery}+${tag.toLowerCase()}`} target="_blank" rel="noopener noreferrer" className="text-xs px-2.5 py-1 bg-white/60 dark:bg-slate-800/60 rounded-full text-slate-600 dark:text-slate-300 hover:bg-red-100 dark:hover:bg-red-900/40 border border-red-200/40 dark:border-red-800/40 transition-colors">
                {tag}
              </a>
            ))}
          </div>
