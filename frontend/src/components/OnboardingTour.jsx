    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="glass-card rounded-3xl shadow-2xl max-w-md w-full mx-4 p-8 animate-scaleIn">
        <div className="flex flex-col items-center text-center">
          <div className="w-20 h-20 bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/50 dark:to-rose-900/50 rounded-2xl flex items-center justify-center text-4xl mb-5 shadow-lg">
            {steps[step].emoji}
          </div>
          ...
          <div className="flex gap-2 mb-8">
            {steps.map((_, i) => (
              <div key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === step ? 'w-6 bg-red-600' : 'bg-stone-300 dark:bg-stone-600'}`} />
            ))}
          </div>
          ...
          <div className="flex gap-3 w-full">
            ...
            {step < steps.length - 1 ? (
              <button onClick={() => setStep(s => s + 1)} className="flex-1 px-4 py-2.5 btn-glass btn-glass-red rounded-xl text-sm font-semibold">
                Next
              </button>
            ) : (
              <button onClick={dismiss} className="flex-1 px-4 py-2.5 btn-glass btn-glass-red rounded-xl text-sm font-semibold">
                Get Started 🚀
              </button>
            )}
          </div>
