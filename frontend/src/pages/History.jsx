      ...
      {selected && r ? (
        <div>
          ...
          {r.improvements?.overall_verdict && (
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/40 dark:to-teal-900/40 border border-emerald-200/60 dark:border-emerald-700/30 rounded-2xl p-5 mb-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {r.scan_summary.items.map(item => (
                  <div key={item.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-3">
                    ...
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full inline-block mt-1 ${item.freshness_status === 'Fresh' ? 'bg-emerald-100 dark:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300' : item.freshness_status === 'Slightly Aged' ? 'bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300' : 'bg-red-100 dark:bg-red-900/60 text-red-700 dark:text-red-300'}`}>{item.freshness_status}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-3">
          {history.map((entry) => {
            ...
            return (
              <button key={entry.id} onClick={() => setSelected(entry)} className="w-full text-left bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group">
