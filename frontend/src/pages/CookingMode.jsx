    ...
    <>
      ...
      <header className="glass-nav px-6 py-4 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-gradient-to-br from-red-500 to-red-600 rounded-xl flex items-center justify-center text-sm shadow-lg shadow-red-600/20">👨‍🍳</div>
        ...
        <aside className="lg:w-80 glass-card p-6 border-b lg:border-b-0 lg:border-r">
          ...
          <div className="space-y-2.5">
            {ingredients.map((ing, i) => (
              <button key={i} onClick={() => toggleIngredient(i)} className={`w-full text-left px-4 py-3 rounded-xl text-base flex items-center gap-3 transition-all ${checked.includes(i) ? 'bg-stone-700/50 text-stone-400 line-through' : 'bg-stone-700/30 hover:bg-stone-700/60 border border-stone-700/40 hover:border-stone-600/60'}`}>
                <span className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${checked.includes(i) ? 'bg-red-600 border-red-600 shadow-sm' : 'border-stone-500'}`} role="checkbox" aria-checked={checked.includes(i)}>
                  {checked.includes(i) ? '✓' : ''}
                </span>
                {ing}
              </button>
          ...
          <div className="mt-8 pt-6 border-t border-stone-700/60">
            ...
            <div className="flex gap-2 flex-wrap">
              {[1, 2, 3, 5, 10, 15].map(m => (
                <button key={m} onClick={() => startTimer(m)} className="px-3 py-2 bg-stone-700/50 rounded-xl text-sm hover:bg-stone-600/60 transition-all border border-stone-600/40 hover:border-red-500/40 hover:shadow-lg hover:shadow-red-600/10 active:scale-95">
                  {m} min
                </button>
              ))}
            </div>
            {timerSeconds > 0 && (
                <div className="mt-5 text-center bg-stone-700/30 rounded-xl p-4 border border-stone-600/40">
                <p className="text-4xl font-mono font-bold text-red-400">{Math.floor(timerSeconds / 60)}:{(timerSeconds % 60).toString().padStart(2, '0')}</p>
          ...
          <div className="max-w-2xl w-full">
            <div className="text-center mb-8">
              <div className="flex items-center justify-center gap-2 mb-3">
                ...
                <span className="text-sm font-bold text-red-400">{currentStep + 1}</span>
                <span className="text-xs text-stone-500">/</span>
                <span className="text-sm text-stone-400">{steps.length}</span>
              </div>
              <div className="w-full bg-stone-700/60 rounded-full h-2 overflow-hidden shadow-inner" role="progressbar" aria-valuenow={currentStep + 1} aria-valuemin={1} aria-valuemax={steps.length} aria-label={`Step ${currentStep + 1} of ${steps.length}`}>
                <div className="bg-gradient-to-r from-red-500 to-red-600 h-2 rounded-full transition-all duration-500 ease-out shadow-sm" style={{ width: `${((currentStep + 1) / steps.length) * 100}%` }} />
              </div>
            </div>
            ...
            <div className="flex justify-between gap-4">
              <button
                ...
                className="flex-1 px-6 py-4 bg-stone-700/60 backdrop-blur-sm rounded-xl text-lg font-semibold disabled:opacity-30 hover:bg-stone-600/60 transition-all border border-stone-600/40 hover:border-stone-500/40 active:scale-[0.98]"
                aria-label={t('cookingMode.prev')}
              >
                ← {t('cookingMode.prev')}
              </button>
              {currentStep === steps.length - 1 ? (
                <button onClick={() => navigate(-1)} className="flex-1 px-6 py-4 btn-glass btn-glass-red rounded-xl text-lg font-semibold active:scale-[0.98]">
                  {t('cookingMode.done')} ✓
                </button>
              ) : (
                <button
              ...
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border ${speaking ? 'bg-red-600/80 border-red-500/50 hover:bg-red-500/80 shadow-lg shadow-red-600/20' : 'bg-stone-700/50 border-stone-600/40 hover:bg-stone-600/50'}`}
                  >
                    {speaking ? '🔊' : '🔇'} {speaking ? t('voiceAssistant.stopReading') : t('voiceAssistant.readAloud')}
                  </button>
                  <button
                    onClick={() => setListening(l => !l)}
                    className={`px-4 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border ${listening ? 'bg-red-600/80 border-red-500/50 hover:bg-red-500/80 shadow-lg shadow-red-500/20 animate-pulse' : 'bg-stone-700/50 border-stone-600/40 hover:bg-stone-600/50'}`}
                  >
                    🎤 {t('voiceAssistant.voiceCommands')}
                  </button>
                </div>
                {listening && <span className="text-xs text-red-400 animate-pulse font-medium">{t('voiceAssistant.listening')}</span>}
              </div>
              {voiceInput && (
                <div className="mt-3 text-sm text-stone-300 bg-stone-700/40 backdrop-blur-sm rounded-xl px-4 py-2.5 border border-stone-600/40">
                  🗣 "{voiceInput}"
                </div>
              )}
