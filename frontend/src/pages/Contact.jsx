        ...
        <div className="grid md:grid-cols-2 gap-10 mb-16">
          <div className="glass-card rounded-2xl shadow-xl p-8">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-11 h-11 bg-gradient-to-br from-orange-400 to-orange-500 rounded-xl flex items-center justify-center text-lg shadow-md">✉️</div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea name="message" value={form.message} onChange={handleChange} required rows="5" className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-stone-700/80 border border-stone-200/80 dark:border-stone-600/60 text-stone-800 dark:text-stone-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-400/60 focus:border-orange-400 transition-all resize-vertical shadow-sm" placeholder={t('contact.message')} />
              </div>
              <button type="submit" className="w-full py-3 btn-glass btn-glass-orange rounded-xl active:scale-[0.98]">{t('contact.submit')}</button>
            </form>
          </div>
              ...
              {faqItems.map((item, i) => (
                <div key={i} className="glass-card rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
                  <button onClick={() => setExpanded(expanded === i ? null : i)} className="w-full flex items-center justify-between p-5 text-left hover:bg-stone-50/80 dark:hover:bg-stone-700/40 transition-colors">
                    ...
                    <span className={`shrink-0 w-7 h-7 rounded-full bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/50 dark:to-teal-900/50 flex items-center justify-center text-sm font-bold text-orange-600 dark:text-orange-400 transition-all duration-300 border border-orange-200/60 dark:border-orange-700/50 ${expanded === i ? 'rotate-180 bg-gradient-to-br from-emerald-100 to-teal-100 dark:from-emerald-800/60 dark:to-teal-800/60' : ''}`}>
