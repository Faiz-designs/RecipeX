    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="bg-gradient-to-br from-emerald-600 via-emerald-500 to-teal-600 rounded-2xl p-6 md:p-8 mb-8 text-white">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold">Shopping List</h1>
            <p className="text-emerald-100/80 text-sm">{items.length} item{items.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
        {items.length > 0 && (
          <div className="mt-4 bg-white/10 rounded-xl p-3">
            <div className="flex justify-between text-sm mb-1">
              <span className="font-medium">{checkedCount}/{items.length} checked</span>
      ...
      <div className="flex gap-2 mb-6">
        ...
        />
        <button onClick={handleAdd} className="px-5 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-bold hover:from-emerald-600 hover:to-teal-700 transition-all shadow-md hover:shadow-lg active:scale-[0.98]">Add</button>
      </div>

      {items.length === 0 ? (
        <div className="text-center py-16">
          <div className="text-5xl mb-4">🛒</div>
          <p className="text-lg font-medium text-slate-500 dark:text-slate-400 mb-2">Your list is empty</p>
          <p className="text-sm text-slate-400 dark:text-slate-500 mb-6">Add ingredients or scan vegetables and add recipe ingredients</p>
          <Link to="/scan" className="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-6 py-3 rounded-xl font-bold shadow-md hover:shadow-lg transition-all">Scan Vegetables →</Link>
        </div>
      ) : (
      ...
      {items.length > 0 && (
        <div className="flex justify-between items-center mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
          <span className="text-xs text-slate-400 dark:text-slate-500">{items.length - checkedCount} remaining</span>
          <button onClick={handleClear} className="text-xs text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 font-medium transition-colors">Clear All</button>
        </div>
      )}
    </div>
  )
