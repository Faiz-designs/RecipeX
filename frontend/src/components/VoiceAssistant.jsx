...
export default function VoiceAssistant() {
  ...
  const { isListening, transcript, isSupported, lastCommand, toggleListening, submitText } = useVoiceAssistant()
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {!isSupported && (
        <div className="glass-card rounded-2xl px-4 py-3 shadow-xl border border-amber-500/30 bg-stone-900/90 backdrop-blur-xl">
          <p className="text-xs text-amber-400">Voice not supported. Type commands below.</p>
        </div>
      )}
      {isListening && (
      ...
      {lastCommand && !isListening && (
        <div className="glass-card rounded-2xl px-4 py-2.5 shadow-xl border border-stone-600/30 bg-stone-900/90 backdrop-blur-xl max-w-[260px]">
          <p className="text-xs text-stone-400">{lastCommand}</p>
        </div>
      )}
      <div className="relative flex flex-col items-end gap-2">
        {showHint && !isListening && !textMode && (
          <div className="absolute bottom-16 right-0 glass-card rounded-2xl px-4 py-3 shadow-xl border border-lime-500/20 bg-stone-900/90 backdrop-blur-xl min-w-[200px] mb-1">
            <p className="text-xs text-stone-300 leading-relaxed">
              Say: &ldquo;prepare tomato salad&rdquo;<br />
        ...
        <div className="flex gap-2 items-end">
            ...
            <form onSubmit={handleTextSubmit} className="glass-card rounded-2xl p-2 shadow-xl border border-lime-500/20 bg-stone-900/95 backdrop-blur-xl">
              <div className="flex gap-2">
                <input
                  ...
                  className="px-3 py-2 bg-stone-800 border border-stone-700 rounded-xl text-sm text-stone-200 placeholder-stone-500 focus:outline-none focus:border-lime-500/50 w-44"
                  autoFocus
                />
                <button type="submit" className="px-3 py-2 bg-lime-600 rounded-xl text-sm font-semibold text-white hover:bg-lime-500 transition-all">
                  Go
                </button>
              </div>
            </form>
          ...
          <div className="flex gap-2">
            {isSupported && (
              <button
                onClick={() => { setShowHint(false); setTextMode(false); toggleListening() }}
            ...
            <button
              onClick={() => { setTextMode(t => !t); setShowHint(false) }}
